                                                              ...
                                                              uint8_t channel)
{
  ...
  // The exitMode enumuration type is mapped as [b3b2b1] = [b3b2] | [b1] of `options` feilds in the Gp Proxy Commissioning Mode
  // commaand where (b3-b2) for `ExitMode` feilds and b1 is the exit on commissioning window time expiry.
...
void emberDGpSentHandler(EmberStatus status, uint8_t gpepHandle)
{
  if (status == EMBER_SUCCESS
    ...
    emberStopScan();
    commissioningState.channelStatus = 0;
    // Here it should clear the 5 sec timer that started while submitted a packet to TxQueue
    emberAfPluginGreenPowerClientChannelEventHandler();
  }
}
