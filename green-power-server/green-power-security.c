...
#define MAKE_HEADER_FLAG_NWK_FC_EXT(direction, keyType, securityLevel, appId) ((direction << 7) + (keyType << 5) + (securityLevel << 3) + appId)
                             ...
                             uint32_t gpdSecurityFrameCounter)
{
  ...
  header[headerLength++] = MAKE_HEADER_FLAG_NWK_FC_EXT(direction, keyType, securityLevel, gpdAddr->applicationId);
  if (gpdAddr->applicationId == GPD_APPLICATION_ID_IEEE) {
...
//   gpdCommandPayload
//   securedOutgoingGpdf
//   securedOutgoingGpdfMaxLength
// Outputs :
//   securedOutgoingGpdf
// Returns :
//   length of the secured GPDF
                                   ...
                                   uint8_t securedOutgoingGpdfMaxLength)
{
  ...
  if (securityLevel == EMBER_GP_SECURITY_LEVEL_FC_MIC_ENCRYPTED) {
    ...
    //emberAfGreenPowerClusterPrint("Encrypted Payload :[");
    //for (int i = 0; i < payloadLength; i++) {
    // emberAfGreenPowerClusterPrint("%x ",securedOutgoingGpdf[encryptionStartIndex + i]);
    //}
